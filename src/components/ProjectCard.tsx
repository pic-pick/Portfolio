...
function ProjectCard({ project, onOpen }: ProjectCardProps) {
    const isOpenable = Boolean(project.detailType || project.pdfUrl)
    ...
    return (
        <article
            className={`group flex h-full flex-col border-b border-zinc-200 pb-6 transition duration-300 hover:translate-x-1 hover:-translate-y-0.5 ${
        ...
        >
            ...
            <button
                type="button"
                onClick={handleOpen}
                className={`mt-auto pt-6 flex w-full items-center justify-between text-sm font-medium transition duration-300 ${
                    isOpenable
                        ? "text-zinc-900 hover:text-zinc-600"
                        : "cursor-default text-zinc-400"
                }`}
            >
                <span>{isOpenable ? "자세히 보기" : "준비 중"}</span>
                <span
                    className={`transition duration-200 ${
                        isOpenable ? "group-hover:translate-x-1" : ""
                    }`}
                >
          →
        </span>
